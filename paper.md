...
However, these architectures are generic and not explicitly designed for thermal imaging.
...
| :--- | :---------------------------------------------- | :------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.   | Modified CNN (Cloud_Res) (Jalil et al., 2023)   | Two thermal DBs (Tufts + other); 3366 images; Cloud environment      | 99% (Overall Accuracy; *achieved on specific dataset setup; TH-SE-ResNet tested across multiple datasets*)                                                                     |
| 2.   | ResNet-50 (Best Model) (Chatterjee & Zaman, 2023)| Tufts and Charlotte; Used Kalman filtering preprocessing             | 95.0% Accuracy (*TH-SE-ResNet exceeded this [up to 97%] on same datasets via architectural enhancements*)                                                                     |
...
| 4.   | VGG-19 (Best Model) (Prihodova, 2022)           | **Fused Multispectral (MSX) Hand Images (Palm)**; 46 subjects        | **94.9% Accuracy** (*High accuracy, but uses **fused Visible+Thermal Hand Images (MSX)**, a different modality & input type than TH-SE-ResNet's thermal-only facial focus*)              |
| 5.   | AlexNet (Gurram et al., 2024)                   | Thermal Images (Tufts with augmentation, 2031 images)                           | **82.6% Accuracy** (*Significantly lower than TH-SE-ResNet, confirming limitations of older/shallower architectures on this thermal data*)                                         |
| 6.   | InceptionV3 (Gurram et al., 2024)               | Thermal Images (Tufts with augmentation, 2031 images)                           | **92.3% Accuracy** (*Good performance, but TH-SE-ResNet achieves higher results [up to 97% on Tufts] with specific thermal adaptations*)                                           |
| 7.   | IRT_ResNet-101 (Best Model) (Jilal & Reda, 2022) | Two thermal DBs (Tufts + other); 3366 images                          | **99% (Overall Accuracy - Combined datasets)**; *Very high result on their combined data test, though performance varied between genders (100% Male/98.2% Female); TH-SE-ResNet also achieved strong accuracy (up to 97%/90%) with **more balanced gender metrics across various dataset configurations**.* |
| 8.   | VGG-19 (Best Model) (Prihodova & Jech, 2024)    | Thermal **Facial** Images (Train: FATFD, Test: **Drone** OCD); Outdoor | **85.5% Accuracy** (*Moderate accuracy reflecting challenging outdoor **drone-based capture** with lower resolution sensor; TH-SE-ResNet achieved higher accuracy on static data*)       |
| 9.   | **TH-SE-ResNet (This Paper)**                   | **Tufts / Charlotte / Combined** (Thermal Facial); Std preprocessing | **Up to 97% (Tufts), 90% (Combined)**; *Demonstrates SOTA accuracy & superior generalization on **thermal-only facial data** via tailored architecture (SE Blocks, Channel Adapter)* |
